...
export default function Block() {
  ...
  const poolReward = mojo_to_lotus(calculatePoolReward(blockRecord.height));
  const baseFarmerReward = mojo_to_lotus(
    calculateBaseFarmerReward(blockRecord.height),
  );

  const lotusFees = blockRecord.fees
    ? mojo_to_lotus(BigInt(blockRecord.fees))
    : '';
  ...
  const rows = [
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.lotusexplorer.com/blockchain/puzzlehash/${blockRecord.farmer_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
    ...
    },
    {
      ...
      value: (
        <Link
          target="_blank"
          href={`https://www.lotusexplorer.com/blockchain/puzzlehash/${blockRecord.pool_puzzle_hash}`}
        >
          {currencyCode
            ? toBech32m(
